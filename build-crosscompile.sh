...
./configure --prefix=$TARGET_DIR --host=x86_64-apple-darwin11 --with-sysroot=/SDKs/MacOSX.platform/MacOSX10.6.sdk --enable-static --disable-shared
make -j 4 && make install
...
./configure --prefix=$TARGET_DIR --target=x86_64-darwin11 --host=x86_64-apple-darwin11 --enable-static --disable-shared
make -j 4 && make install
...
./configure --prefix=$TARGET_DIR --target=x86_64-darwin11-gcc --sdk-path=/SDKs/MacOSX.platform/MacOSX10.6.sdk --enable-runtime-cpu-detect --enable-static --disable-shared
make -j 4 && make install
...
CFLAGS="-I$TARGET_DIR/lavf/include" LDFLAGS="-L$TARGET_DIR/lavf/lib -framework CoreFoundation -framework CoreVideo -framework VideoDecodeAcceleration" ./configure --prefix=$TARGET_DIR --host=x86_64-apple-darwin11 --enable-static --disable-asm --sysroot=/SDKs/MacOSX.platform/MacOSX10.6.sdk
make -j 4 && make install

echo "*** Building xvidcore ***"
cd "$BUILD_DIR/xvidcore/build/generic"
./configure --prefix=$TARGET_DIR --host=x86_64-apple-darwin11 --enable-static --disable-shared
make -j 4 && make install
